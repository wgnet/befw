...
func PortFromTag(tag string) (*legacyBefwPort, error)  {
	p := strings.Split(tag, "/")
	if len(p) != 2 {return nil, fmt.Errorf("Expected format: '<port>/<protocol>'.")}

	dport := portRange(p[0])
	proto := befwServiceProto(strings.ToLower(p[1]))

	if proto != ipprotoTcp && proto != ipprotoUdp {return nil, fmt.Errorf("Expected protocol: 'tcp' or 'udp' in '<port>/<protocol>'.")}
...
func  LegacyServiceFromJson(data []byte) (*legacyService, error) {
		...
		for _, rawPort := range v.RawServicePorts {
            ...
            case map[string]interface{}:
                ...
                if n, ok := mapPort["port"]; ok {
                    switch n.(type) {
                        case float64: port = fmt.Sprintf("%d", uint16(n.(float64)))
                        case string: port = n.(string)
                    }
                }
                if v, ok := mapPort["protocol"]; ok { prot = v.(string) }
                var err error
    ...
    if len(v.ServicePorts) > 0 && first == nil { first = &v.ServicePorts[0] }
	...
	if v.ServicePort == 0 {
		if first == nil { return nil, fmt.Errorf("Expected not empty 'port' or 'ports' fields.") }
		port := strings.Split(string(first.Port), ":")
		value, err := strconv.Atoi(port[0])
		if err != nil { return nil, fmt.Errorf("Bad port in ports.") }
