...
func TestIpsetV6(t *testing.T) {
    ...
    err := fw.Apply(&s)
    if err != nil { t.Error(err.Error()); t.FailNow() }

    // Expected
    if !strings.Contains(result, name) { t.Errorf("Expect name in result:\n%s", result) }
    if !strings.Contains(result, "4.4.4.0/28") { t.Errorf("Expect correct IPv4 mask in result:\n%s", result) }

    if !strings.Contains(result, name+V6) { t.Errorf("Expect name_v6 in result:\n%s", result) }
    if !strings.Contains(result, "family inet6") { t.Errorf("Expect family inet6 mask in result:\n%s", result) }
    if !strings.Contains(result, "::1:0:0:0/80") { t.Errorf("Expect correct IPv6 mask in result:\n%s", result) }
}
...
func TestIptablesV6(t *testing.T) {
    ...
    err := fw.Apply(&s)
    if err != nil { t.Fail() }
    if !strings.Contains(result, name) { t.Errorf("Expect '%s' in:\n%s", name, result) }
    if !strings.Contains(resultv6, name + V6) { t.Errorf("Expect '%s' in:\n%s", name+V6, resultv6) }
}
...
func asService(name string, ports []string, clients []string) bService {
    ...
    for _, cl := range clients {
        ...
        if strings.HasPrefix(cl, "5.1") { tm -= 2000 }                      // Expired
        _, r, _ := net.ParseCIDR(cl)
        //if r == nil { _, r, _ = net.ParseCIDR(cl + "/32") }               // Try as IP
        if r == nil { continue }
