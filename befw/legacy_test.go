...
func TestUnmarshalJson(t *testing.T) {
	...
	if srv.Name != "example" {
		t.Error("Wrong name", srv.Name)
	}
	score := 0
	for _, p := range srv.Ports {
		if p.toTag() == "12345/tcp" {
			score += 1
		}
		if p.toTag() == "1:42/udp" {
			score += 1
...
func TestUnmarshalJsonLegacy(t *testing.T) {
	...
	if service.ServiceName != "example" {
		t.Errorf("Bad name: %s != %s", service.ServiceName, "example")
	}
	if service.ServicePort != 12345 {
		t.Errorf("Bad port")
	}
	if service.ServiceProtocol != "tcp" {
		t.Errorf("Bad protocol")
	}
	...
	if len(service.ServicePorts) != 1 && string(service.ServicePorts[0].Port) != "1:42" {
		t.Errorf("Bad ports unmarshalling")
	}
	...
	if service.ServicePort != 11 {
		t.Errorf("Bad port")
	}
	if service.ServiceProtocol != "udp" {
		t.Errorf("Bad protocol")
	}
	...
	if err == nil || service != nil {
		t.Errorf("Expected FAIL on unmarshalling bad JSON")
	}
	...
	service, err = LegacyServiceFromJson([]byte(jsnService))
	if err == nil {
		t.Errorf("Expected error for : %s", jsnService)
		return
	}
...
func TestUnmarshalJsonLegacy2(t *testing.T) {
	...
	for _, port := range expectPorts {
		...
		for _, realPort := range service.ServicePorts {
			if string(realPort.Port) == bp.Range() && string(realPort.PortProto) == bp.Protocol {
				ok = true
				break
			}
		}
		if !ok {
			t.Errorf("Expect port %s, but exist in %v", bp.toTag(), service.ServicePorts)
		}
	}
	...
	service, err = LegacyServiceFromJson([]byte(jsnService))
	if err == nil {
		t.Errorf("Expect exception, since bad port in json: %s", jsnService)
		return
	}
}
...
func TestTag(t *testing.T) {
	...
	for k, v := range expected {
		...
		if port.toTag() != expected {
			t.Errorf("Wrong tag '%s'; Expected: '%s'", port.toTag(), expected)
		}
	}
	...
	for _, tag := range tags {
		...
		if err != nil || newPort == nil {
			t.Errorf("Failed PortFromTag result")
		}
		p := strings.Split(tag, "/")
		if string(newPort.Port) != p[0] || string(newPort.PortProto) != p[1] {
			t.Errorf("Wrong parsed port and proto: %s %s", newPort.Port, newPort.PortProto)
		}
	}
	...
	for _, tag := range bads {
		...
		if err == nil || port != nil {
			t.Errorf("Expected error for bad tag %s", tag)
		}
	}
}
